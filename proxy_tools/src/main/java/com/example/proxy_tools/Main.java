...
public class Main {
    ...
    private static String dex = "D:/Android/sdk/build-tools/30.0.2/";
...
//        System.out.println(process.waitFor() == 0 ? "zipalign成功" : "zipalign失败");
    ...
    public static void jksToApk() throws IOException, InterruptedException {
        ...
        File jks = new File("proxy_tools/norman.jks");
        File alignedApk = new File("app/build/outputs/apk/debug/app-unsigned-aligned.apk");
        ...
        String text = "cmd /c" + dex + "apksigner sign --ks " + jks.getAbsolutePath()
                + " --ks-key-alias norman --ks-pass pass:norman --key-pass pass:norman --out "
                + signedApk.getAbsolutePath() + " " + alignedApk.getAbsolutePath();
        Process process =  Runtime.getRuntime().exec(text);
        process.waitFor();
        if (process.exitValue() != 0) {
